...
(define (rectangle-inside-polygon? rect polygon)
  ...
  (and (point-not-outside? (Pos x1 y1) polygon)
       ...
       (line-not-outside? x1 y1 x1 y2 polygon)
       (line-not-outside? x1 y2 x2 y2 polygon)
       (line-not-outside? x2 y2 x2 y1 polygon)
       (line-not-outside? x2 y1 x1 y1 polygon)))
...
(define (point-not-outside? point polygon)
  (or (point-on-edge? point polygon)
      (point-inside? point polygon)))
...
(define (point-on-edge? point polygon)
  (match-define (Pos x y) point)
  (for/or ([v1 (in-list polygon)]
           [v2 (in-list (cdr polygon))])
    (match-define (Pos x1 y1) v1)
    (match-define (Pos x2 y2) v2)
    (or (and (= x1 x x2) (<= (min y1 y2) y (max y1 y2)))
